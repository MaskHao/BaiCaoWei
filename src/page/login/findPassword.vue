<template>
    <div>
        ...
        <div class="login-box">
            ...
            <div class="input-btn" @click="changePwdFun()">完成</div>
        </div>
...
<script>
    ...
    import {sendPhoneCode,findPassword} from '../../service/getData'

    export default {
        data () {
            return {
                pageTitle: this.$route.query.type == 1 ? '找回密码' : '重置密码',
                userPhone: '',
                userPwd: '',
                userYzm: '',
                yzmText: '获取验证码',
                yzmState: 0,
                yzmType: this.$route.query.type
            }
        },
        components: {
            vAlert
        ...
        methods:{
            sendCode(){
                var that = this;
                var phone = /^1(3[0-9]|4[57]|5[0-35-9]|7[678]|8[0-9])[0-9]{8}$/;
                var _userPhone = that.trim(this.userPhone);
                if(phone.test(_userPhone)){
                    var time = 60;
                    if(that.yzmState == 0){
                        that.yzmState = 1;
                        sendPhoneCode(_userPhone,that.yzmType).then(res => {
            ...
            changePwdFun(){
                ...
                if(!phone.test(_userPhone)){
                    ...
                    findPassword(_userPhone , _userPwd,_userYzm).then(response => {
